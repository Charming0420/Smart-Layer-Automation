...
def perform_action(cat_id, action):
    """执行指定行动，动态调整Gas Price，并根据操作类型调整Gas Limit"""
    gas_price = web3.eth.gas_price  # 获取当前推荐的gasPrice
    print(f"Current gas price: {gas_price}")
    txn = None
    
    if action == "clean":
        can_clean = contract.functions.canClean(cat_id).call()
        print(f"Checking if Cat ID {cat_id} can be cleaned: {can_clean}")
        if can_clean:
            txn = contract.functions.cleanCat(cat_id).build_transaction({
                ...
                'gas': 120000,  # 对clean操作设置Gas Limit
                'gasPrice': gas_price
            })
    elif action == "feed":
        can_feed = contract.functions.canFeed(cat_id).call()
        print(f"Checking if Cat ID {cat_id} can be fed: {can_feed}")
        if can_feed:
            txn = contract.functions.feedCat(cat_id).build_transaction({
                'from': my_wallet_address,
                'nonce': web3.eth.get_transaction_count(my_wallet_address),
                'gas': 87000,  # 对feed操作设置Gas Limit
                'gasPrice': gas_price
            })
    # 处理其他操作...
    ...
    if txn is not None:
        try:
            ...
            receipt = web3.eth.wait_for_transaction_receipt(txn_hash, timeout=240)  # 增加等待时间为240秒
            if receipt.status == 1:
        ...
        for cat_id in cat_ids:
            for action in actions:
                if can_perform_action(cat_id, action):
                    ...
                    time.sleep(2)  # 等待一段時間避免過快發送交易
        time.sleep(2)  # 每小時檢查一次
